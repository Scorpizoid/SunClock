...
void CSunClockAppView::Draw(const TRect& /*aRect*/) const
{
	...
	TRect      sideBarRect(0,0,63,240);// = Rect();
	...
	TInt dayNoInMonth = currentTime.DayNoInMonth() + 1;
				...
				// Selvitet‰‰n, monesko kuukauden p‰iv‰ on menossa.
				// Luku alkaa nollasta, joten t‰ytyy lis‰t‰ yksi p‰iv‰.
	TInt dayNoInWeek = currentTime.DayNoInWeek() + 1;
	...
	iYearText.Format(KYear, year);
				...
				// Teksti, joka ilmoittaa tunnin ja minuutin
	_LIT(KMonday,"Monday");
	...
	TBuf<11> timeText;
	if (dayNoInWeek == 1) timeText.Format(KMonday);
	if (dayNoInWeek == 2) timeText.Format(KTuesday);
	if (dayNoInWeek == 3) timeText.Format(KWednesday);
	if (dayNoInWeek == 4) timeText.Format(KThursday);
	if (dayNoInWeek == 5) timeText.Format(KFriday);
	if (dayNoInWeek == 6) timeText.Format(KSaturday);
	if (dayNoInWeek == 7) timeText.Format(KSunday);
	...
	gc.SetPenColor(KRgbWhite); // change the pen color 
				...
				// Viivojen pisteet
	TPoint whiteLineStartPoint[11];
	TPoint whiteLineEndPoint[11];
	...
	whiteLineStartPoint[10].SetXY(0,239);

	whiteLineEndPoint[0].SetXY(62,0);
	whiteLineEndPoint[1].SetXY(62,240);
	whiteLineEndPoint[2].SetXY(63,199);
	whiteLineEndPoint[3].SetXY(0,239);
	...
	whiteLineEndPoint[8].SetXY(1,239);
	whiteLineEndPoint[9].SetXY(59,91);
	whiteLineEndPoint[10].SetXY(62,239);
	...
	for(i=0;i<=10;i++)
	{
		gc.DrawLine(whiteLineStartPoint[i],whiteLineEndPoint[i]); 
			...
			// Harmaat viivat
	TPoint greyLineStartPoint[8];
	TPoint greyLineEndPoint[8];
	...
	greyLineStartPoint[6].SetXY(61,200);
	greyLineStartPoint[7].SetXY(2,238);
	...
	greyLineEndPoint[6].SetXY(61,239);
	greyLineEndPoint[7].SetXY(61,238);
	...
	if ((dayNoInYear >= 1) && (dayNoInYear <= 31)) {
	 ...
	 // Kun halutaan siirt‰‰ varjoa oikealle, v‰hennet‰‰n maskPosition_X:‰‰.
		maskPosition_X = maskPosition_X - 201;//+ 198;//235;
	...
	*/
	...
	CFbsBitmap* bitmapFrame;
	bitmapFrame = new (ELeave) CFbsBitmap();
	CleanupStack::PushL(bitmapFrame); 
				// Ladataan t‰m‰n kuun kuva
	TRAP(err, User::LeaveIfError(bitmapFrame->Load(_L("c:\\system\\apps\\sunclock\\frame.mbm"), 0))); //night.mbm"), 0)));
	if (err)
	{
		TRAP(err, User::LeaveIfError(bitmapFrame->Load(_L("d:\\system\\apps\\sunclock\\frame.mbm"), 0))); //night.mbm"), 0)));
		if (err)
		{
			User::LeaveIfError(bitmapFrame->Load(_L("z:\\system\\apps\\sunclock\\frame.mbm"), 0)); //night.mbm"), 0));
		}
	}
	CleanupStack::Pop();

				// Blitataan kuva sivupalkin viereen.
	TPoint framePosition(63,0); 
	TRect frameRect(0,0,507,240);
	gc.BitBlt(framePosition, bitmapFrame, frameRect);
	...
	CleanupStack::PushL(bitmapNight); 
				// Ladataan t‰m‰n kuun kuva
	TRAP(err, User::LeaveIfError(bitmapNight->Load(_L("c:\\system\\apps\\sunclock\\mapnight.mbm"), 0))); //night.mbm"), 0)));
	if (err)
	{
		TRAP(err, User::LeaveIfError(bitmapNight->Load(_L("d:\\system\\apps\\sunclock\\mapnight.mbm"), 0))); //night.mbm"), 0)));
		if (err)
		{
			User::LeaveIfError(bitmapNight->Load(_L("z:\\system\\apps\\sunclock\\mapnight.mbm"), 0)); //night.mbm"), 0));
		}
	}
	CleanupStack::Pop();

				// Blitataan kuva sivupalkin viereen.
	TPoint nightMapPosition(63+iMapPosition+51,21); 
	TRect nightMapRect(0,0,400-iMapPosition,200);
	gc.BitBlt(nightMapPosition, bitmapNight, nightMapRect);

	TPoint nightMapPosition2(63+51,21);
	...
	TRAP(err, User::LeaveIfError(bitmapSeason->Load(_L("c:\\system\\apps\\sunclock\\map.mbm"), 0)));
	if (err)
	{
		TRAP(err, User::LeaveIfError(bitmapSeason->Load(_L("d:\\system\\apps\\sunclock\\map.mbm"), 0)));
		if (err)
		{
			User::LeaveIfError(bitmapSeason->Load(_L("z:\\system\\apps\\sunclock\\map.mbm"), 0));
		}
	}
	...
	TPoint posSeasonBitmap(63+iMapPosition+51,21); 
	TPoint posSeasonBitmap2(63+51,21); //modulo
...
//	TInt hourOffset = off * 12;
	...
	gc.DrawText(timeText,sideBarRect,baselineOffset6,CGraphicsContext::ECenter, 0);
...
/*
	           // Area in which we shall draw
	TRect      wholeRect(0,0,463,200);// = Rect();
	TPoint screenOrigin(0,0); // top left of the screen
	TSize area1(100,100); // size of area to copy
	wholeRect.SetRect(screenOrigin,area1); // set rectangle to copy
	TRect fillRect(0,0,200,200);
	TPoint offset(0,0); // target area to right of copied area
	gc.CopyRect(offset,wholeRect); // copy rect to target
*/

	/*
	CFbsBitmap *iBitmap=new (ELeave) CFbsBitmap;
	TRect theZoomRect(63,0,463,200);
	TRect fillRect(63,0,556,240);
	TPoint positionBitmap(63,0);
	TSize iBitmapSize(400,200);
	iBitmap->Create(iBitmapSize,EGray16);
	eikonEnv->ScreenDevice()->CopyScreenToBitmap(iBitmap,theZoomRect);
	gc.DrawBitmap(fillRect, iBitmap);
	delete iBitmap;
...
*/
	...
	TPoint equatorStartPoint(63+51,99+21);
	TPoint equatorEndPoint(463+51,99+21);

	if (iShowEquator == 1)
	{
		gc.SetPenColor(TRgb(53,53,53)); // change the pen color
		gc.DrawLine(equatorStartPoint,equatorEndPoint); 
	}

				// Jos k‰ytt‰j‰ on m‰‰ritt‰nyt, ett‰ Kravun k‰‰ntˆpiiri piirret‰‰n,
				// niin sitten se myˆs piirret‰‰n.
	TPoint tropicOfCancerStartPoint(63+51,73+21);
	TPoint tropicOfCancerEndPoint(463+51,73+21);
	...
	if (iShowTropicOfCancer == 1)
	{
		...
		gc.DrawLine(tropicOfCancerStartPoint,tropicOfCancerEndPoint); 
				...
				// niin sitten se myˆs piirret‰‰n.
	TPoint tropicOfCapricornStartPoint(63+51,125+21);
	TPoint tropicOfCapricornEndPoint(463+51,125+21);
	...
	if (iShowTropicOfCapricorn == 1)
	{
		...
		gc.DrawLine(tropicOfCapricornStartPoint,tropicOfCapricornEndPoint); 
				...
				// niin sitten se myˆs piirret‰‰n.
	TPoint arcticCircleStartPoint(63+51,25+21);
	TPoint arcticCircleEndPoint(463+51,25+21);
	...
	if (iShowArcticCircle == 1)
	{
		...
		gc.DrawLine(arcticCircleStartPoint,arcticCircleEndPoint); 
				...
				// niin sitten se myˆs piirret‰‰n.
	TPoint antarcticCircleStartPoint(63+51,173+21);
	TPoint antarcticCircleEndPoint(463+51,173+21);
